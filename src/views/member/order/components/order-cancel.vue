<!--  -->
<template>
  <XtxDialog title="取消订单" v-model:visible="showDialog">
    <!-- 组件内容 -->
    ...
    <!-- 按钮操作 -->
    <template #footer>
      <XtxButton type="gray" @click="showDialog = false" style="margin-right: 20px">取消</XtxButton>
...
export default {
  ...
  setup() {
    ...
    const submit = () => {
      if (!currText.value) return message({ text: '请选择取消订单原因' })
    ...
    return { showDialog, submit, currText, open, cancelReason }
  },
}
</script>
